...
// ─── Schemas ─────────────────────────────────────────────────────────────────
// PLANNER_SCHEMA replaced by per-persona buildPlannerSchema() + compiler.
...
// ─── LLM calls ───────────────────────────────────────────────────────────────
// Planner: flat semantic content only. Edges compiled deterministically.
...
async function run() {
  ...
  // Filter to just the 5 target cases
  const TARGET_CASES = ['C003', 'C005', 'C007', 'C008', 'C009'];
  cases = cases.filter(c => TARGET_CASES.some(t => c.caseId.startsWith(t)));
  ...
  if (!isSmokeTest && cases.length !== 5) {
    throw new Error(`EXP-015 targeted requires exactly 5 cells; got ${cases.length}`);
  }

  if (!isSmokeTest) {
    const uniqueCases = new Set(cases.map(c => c.caseId));
    if (uniqueCases.size !== 5) throw new Error("Duplicate caseIds found");
  }
