  ...
  try {
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      ...
      if (user) {
        ...
        // Deduct credit for this generation
        ...
        const creditData = creditRes as unknown as { success: boolean; balance: number; error?: string };

        if (creditErr || (creditData && creditData.success === false && creditData.error === 'INSUFFICIENT_CREDITS')) {
        ...
        if (creditData && creditData.success) {
          ...
          creditDeducted = true; // mark that deduction succeeded
        }
      }
    }
